...
namespace MyEcommerce
{
    public class Startup
    {
        ...
        private async Task CreateRoles(IServiceProvider serviceProvider)
        {
            ...
            var roleCheck2 = await RoleManager.RoleExistsAsync("Staff");
            var roleCheck3 = await RoleManager.RoleExistsAsync("Customer");
            if (!roleCheck1)
            {
            ...
            var ManagerUserExistenceCheck = await UserManager.FindByEmailAsync("manager@gmail.com");
            if (ManagerUserExistenceCheck == null)
            {
                _ = await UserManager.CreateAsync(ManagerUser, "Look123?");
                _ = await UserManager.AddToRoleAsync(ManagerUser, "Manager");
            }
        }
    }
}
